...
export interface StorageStatusInfo {
  ...
  vercelBlobConnected: boolean;
  totalVolunteers: number;
  supabaseCardsCount: number;
  vercelBlobCardsCount: number;
...
/**
 * Fetch storage status & migration readiness
 */
...
export interface MigrationReport {
  ...
  alreadyMigratedOrSkipped: number;
  failedFiles: Array<{ id: string; url: string; reason: string }>;
  safetyNote?: string;
  error?: string;
}

/**
 * Trigger safe migration from Vercel Blob to Supabase Storage
 */
export async function triggerSupabaseMigration(pin: string): Promise<MigrationReport> {
  try {
    ...
    return {
      success: false,
      message: err?.message || 'Migration network error',
      totalFiles: 0,
      migratedCount: 0,
